...
class ShiftType(Document):
	def process_auto_attendance(self):
		...
		for key, group in itertools.groupby(logs, key=lambda x: (x['employee'], x['shift_actual_start'])):
			...
			attendance_date = '2021-12-03'
			attendance_status, working_hours, late_entry, early_exit, in_time, out_time, late_calculation  = self.get_attendance(single_shift_logs)
			mark_attendance_and_link_log(single_shift_logs, attendance_status, key[1].date(), working_hours, late_entry, early_exit, in_time, out_time, self.name, late_calculation)
	...
	def get_attendance(self, logs):
		...
		if cint(self.enable_entry_grace_period) and in_time and in_time > logs[0].shift_start + timedelta(minutes=cint(self.late_entry_grace_period)):
			late_entry = True
			late_calculation = in_time - logs[0].shift_start
		else:
			late_calculation = "00:00:00"
		...
		if self.working_hours_threshold_for_absent and total_working_hours < self.working_hours_threshold_for_absent:
			late_calculation = "00:00:00"
			return 'Absent', total_working_hours, late_entry, early_exit, in_time, out_time, late_calculation
		if self.working_hours_threshold_for_half_day and total_working_hours < self.working_hours_threshold_for_half_day:
			late_calculation = "00:00:00"
			return 'Half Day', total_working_hours, late_entry, early_exit, in_time, out_time, late_calculation
		return 'Present', total_working_hours, late_entry, early_exit, in_time, out_time, late_calculation
