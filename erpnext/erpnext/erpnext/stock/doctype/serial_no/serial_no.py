...
def validate_serial_no(sle, item_det):
	...
	if item_det.has_serial_no==0:
		...
		if serial_nos:
			if cint(sle.secondary_uom) != flt(sle.secondary_uom):
...
def auto_make_serial_nos(args):
	...
	for serial_no in serial_nos:
		...
		elif args.get('secondary_uom', 0) > 0:
...
def update_serial_nos_after_submit(controller, parentfield):
	stock_ledger_entries = frappe.db.sql("""select voucher_detail_no, serial_no, secondary_uom, warehouse
	...
	for d in controller.get(parentfield):
		...
		for sle in stock_ledger_entries:
			if sle.voucher_detail_no==d.name:
				...
				if not accepted_serial_nos_updated and qty and abs(sle.secondary_uom)==qty \
				...
				elif update_rejected_serial_nos and abs(sle.secondary_uom)==d.rejected_qty \
